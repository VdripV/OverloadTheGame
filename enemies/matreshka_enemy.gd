...
var mini_enemy_scene = preload("res://enemies/mini_ranged_enemy.tscn")
...
func spawn_minis():
	for i in range(2):
		var mini = mini_enemy_scene.instantiate()
		mini.global_position = global_position + Vector3(randf_range(-2, 2), 0.0, randf_range(-2, 2))
