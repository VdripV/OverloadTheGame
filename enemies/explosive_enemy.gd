...
var explosionDamage = 60
var explosionRadius = 6.0
...
func dealExplosionDamage():
	...
	for result in results:
		...
		if collider.is_in_group("Player") and collider.has_method("hit") and not collider.has_method("hit_with_knockback"):
			collider.hit(int(damage))
